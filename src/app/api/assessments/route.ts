  ...
  try {
    ...
    if (resolvedResults.length > 0) {
      ...
      const codeToId: Record<string, string> = Object.fromEntries(
        controls.map((c) => [c.controlCode, c.id])
      );
      const missing = controlCodes.filter((code) => !codeToId[code]);
      if (missing.length > 0) {
        return NextResponse.json(
          { error: `Unknown control codes: ${missing.join(", ")}` },
          { status: 400 }
        );
      }
